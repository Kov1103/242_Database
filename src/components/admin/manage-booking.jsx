  ...
  const handleCloseModal = () => {
    ...
    useEffect(() => {
            const fetchUsers = async () => {
                try {
                    const data = await fetchAllBooking(); // Truyền filters
                    console.log(data);
                    setUsers(data);
                } catch (err) {
    ...
    function BookingItem({ user }) {
        ...
        return (
            ...
            <div className="flex flex-col items-end gap-4">
                ...
                {/* <button className="text-white bg-[#f2ae39] px-4 py-2 rounded-lg" onClick={() => {}}>Chỉnh sửa</button> */}
                <button className="text-white bg-[#1b1b1b] px-4 py-2 rounded-lg" onClick={handleViewDetails}>Xem chi tiết</button>
                </div>
            </div>
            </div>
        )
